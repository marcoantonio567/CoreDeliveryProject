...
function MyProjects() {
  ...
  const load = async () => {
    ...
    if (ids.length) {
      ...
      const vCount: Record<string, number> = {}; const dCount: Record<string, number> = {};
      (vols || []).forEach((v: any) => { vCount[v.project_id] = (vCount[v.project_id] || 0) + 1; });
      (dons || []).forEach((d: any) => { dCount[d.project_id] = (dCount[d.project_id] || 0) + 1; });
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-10">
        ...
        {items.length === 0 ? <p className="text-muted-foreground">Você ainda não criou projetos.</p> : (
