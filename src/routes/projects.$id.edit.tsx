...
function EditProject() {
  ...
  useEffect(() => {
    supabase.from("projects").select("*").eq("id", id).maybeSingle().then(({ data }) => setForm(data));
  }, [id]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("projects").update({
        name: form.name, description: form.description, improvement_type: form.improvement_type,
        location: form.location, images,
      }).eq("id", id);
