...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import('../_tanstack-start-manifest_v-DZAuv3yM.mjs');
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import('./router-BVHoRukv.mjs').then(n => n.r),
    import('./start-CJqFzAH7.mjs'),
    import('./empty-plugin-adapters-Ch7xk_sZ.mjs')
  ]);
  return {
    routerEntry,
    startEntry,
...
export { createMiddleware as c, createServerEntry, server as default };
