...
const setupPendingTimeout = (inner, matchId, route, match) => {
  ...
  if (!!(inner.onReady && false && !resolvePreload(inner, matchId) && (route.options.loader || route.options.beforeLoad || routeNeedsPreload(route)) && typeof pendingMs === "number" && pendingMs !== Infinity && (route.options.pendingComponent ?? inner.router.options?.defaultPendingComponent))) {
...
const loadRouteMatch = async (inner, matchPromises, index) => {
  async function handleLoader(preload, prevMatch, previousRouteMatchId, match2, route2) {
    ...
    if (preload && route2.options.preload === false) ; else if (loaderShouldRunAsync && !inner.sync && shouldReloadInBackground) {
...
export { getOrigin as A, BaseRootRoute as B, normalizeSsrResponse as C, attachRouterServerSsrUtils as D, defineHandlerCallback as E, createSerializationAdapter as F, createRawStreamRPCPlugin as G, isResolvedRedirect as H, replaceSsrResponse as I, mergeHeaders as J, executeRewriteInput as K, stripSsrResponseBody as L, defaultSerovalPlugins as M, makeSerovalPlugin as N, getStylesheetHref as O, isSsrResponse as P, RouterCore as R, isDangerousProtocol as a, BaseRoute as b, isModuleNotFoundError as c, deepEqual as d, exactPathTest as e, functionalUpdate as f, isNotFound as g, hasKeys as h, invariant as i, getScrollRestorationScriptForRouter as j, rootRouteId as k, isServer as l, isRedirect as m, createNonReactiveReadonlyStore as n, createNonReactiveMutableStore as o, escapeHtml as p, getAssetCrossOrigin as q, removeTrailingSlash as r, getScriptPreloadAttrs as s, appendUniqueUserTags as t, resolveManifestCssLink as u, transformReadableStreamWithRouter as v, createSsrStreamResponse as w, transformPipeableStreamWithRouter as x, resolveManifestAssetLink as y, getNormalizedURL as z };
